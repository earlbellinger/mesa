...
for (col_name in c("csound", "logRho", "pressure", "temperature")) {
    ...
    x_max <- 1
    for (simulation in dirs) {
        #profile_index <- file.path(simulation_dir, simulation, 
        #                           'LOGS', 'profiles.index')
        #profiles <- read.table(profile_index, skip=1)
        #profile_no <- profiles[which(profiles[,1]==model_no), 3]
        #data_file <- file.path(simulation_dir, simulation, 'LOGS', 
    ...
    cairo_pdf(file.path(plot_dir, paste0('profile_diff_', col_name, '.pdf')), 
    ...
    core_max <- max(rel_diff[1:20,])
    r_max <- min(which(rel_diff[20:nrow(rel_diff),]>core_max, 
                 arr.ind=TRUE)[,1])+20
    x_max <- max(approx_xout[1:r_max])
    y_max <- max(rel_diff[1:r_max])+.01*max(rel_diff[1:r_max])
    for (sim_i in 1:ncol(interps)) {
        if (sim_i == 1) {
            par(bty="l", las=1, mar=mar, mgp=mgp)
    ...
    legend(per_off*x_max, y_max, legend=labels, col=bcl, lty=1, bty='n')
    dev.off()
}
