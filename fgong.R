...
for (exp_dir in list.dirs(fgong_dir, recursive=FALSE)) {
    for (ev_stage_dir in list.dirs(exp_dir, recursive=FALSE)) {
        ...
        widths <- c(.44, .44, .12)
        start_dev("Frequency differences in", "diffs", 
        ...
        for (l_mode in 0:3) {
            ...
            for (model_i in fs) {
                ...
                if (counter == 1) {
                    ...
                    points(relation, pch=20, cex=0.01, col=tcl[counter])
                }
                counter <- counter + 1
            }
            if (grepl('solar-like', ev_stage)) {
                ...
                relation <- ell[,3] ~ ell[,2]
                points(relation, pch='o', cex=0.5, col="black")
                ref <- ell
            }
            ...
            for (model_i in fs) {
                ...
                for (x in n_range) {#x_min:x_max) {
                    #print(c(x_counter, counter, max(ref[,2])))
                    diffs[x_counter, counter] <- 
                        if (x%in%ref[,2] && x%in%ell[,2]) {
                            as.numeric((ell[ell[,2]==x,][1,3] -
                                        ref[ref[,2]==x,][1,3]) / 
                                        ref[ref[,2]==x,][1,3])
                            } else NA
