...
get_obs <- function(profile_header, freqs, history) {
    ...
    obs.DF["g"] <- hstry_info$log_g
    obs.DF["surf_z"] <- hstry_info$log_surf_z
...
parse_dir <- function(directory) {
    ...
    obs.DF <- do.call(rbind, Map(function(profile_file, freqs_file) {
            print(profile_file)
...
boxplot(data.frame("Linear Model"=He.lm.resid, 
    ...
    ylab="Residuals", main="Y ~ observables", pch=3, cex=0.25)
abline(h=0, lty=2, col="red")
dev.off()
...
age.fmla <- log10(age) ~ nu_max + log_g + log_surf_z + L + Teff + 
...
for (cyg in c("16CygA", "16CygB")) {
    ...
    for (i in 1:5000) {
        ...
        ages <- c(ages, 
            10**predict(age.nn, merge(rbind(obs.DF), rbind(seis.DF))))
    }
    
    cairo_pdf(file.path(plot_dir, paste0('age_', cyg, '.pdf')),
        width=plot_width, height=plot_height, family=font)
    hist(ages, breaks=50, main=cyg, xlab="Age [ years ]")
    abline(v=median(ages), col='red')
    abline(v=median(ages)+mad(ages), col='red', lty=2)
    abline(v=median(ages)-mad(ages), col='red', lty=2)
    text(#diff(par("usr")[1:2])/2, diff(par("usr")[3:4])/2,
        4100000000, 150,
        paste("Age:", formatC(median(ages)/1e9), 
              "\n           +/-", formatC(mad(ages)/1e9), "Gyr"))
    dev.off()
